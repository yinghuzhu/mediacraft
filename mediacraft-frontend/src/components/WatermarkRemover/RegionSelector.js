...
export default function RegionSelector({ taskUuid, frameData, onRegionsSelected }) {
  ...
  useEffect(() => {
    if (frameData && frameData.frame_image) {
      const img = new Image();
      img.src = frameData.frame_image;
      img.onload = () => {
  ...
  const handleSubmitRegions = async () => {
    ...
    try {
      ...
      if (response.data && response.data.code === 10000) {
        onRegionsSelected(response.data.data);
      } else {
        setError(response.data?.message || t('watermarkRemover.errors.regionsSubmitFailed'));
      }
    } catch (err) {
