...
export default function ProcessingStatus({ taskUuid }) {
  ...
  const checkStatus = async () => {
    try {
      const response = await watermarkService.getTaskStatus(taskUuid);
  ...
  const getStatusText = () => {
    switch (status.status) {
      case 'processing':
        return t('watermarkRemover.processing.inProgress');
      case 'completed':
        return t('watermarkRemover.processing.completed');
      case 'failed':
        return t('watermarkRemover.processing.failed');
      default:
        return t('watermarkRemover.processing.unknown');
    }
  };
  
  const handleDownload = () => {
    const downloadUrl = watermarkService.getDownloadUrl(taskUuid);
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h2 className="text-xl font-semibold">{t('watermarkRemover.processing.title')}</h2>
      </div>
          ...
          {status.status === 'processing' ? (
            ...
            <>
              ...
              <Button onClick={handleDownload}>
                ...
                {t('watermarkRemover.processing.download')}
              </Button>
            </>
          ) : (
            <>
